...
typedef struct _METADATA_PREVIEWAGGREGATION {
  ...
  METADATA_SRATIONAL
      WhiteBalanceGain_R; //  MF_CAPTURE_METADATA_WHITEBALANCE_GAINS
  METADATA_SRATIONAL
      WhiteBalanceGain_G; //  MF_CAPTURE_METADATA_WHITEBALANCE_GAINS
  METADATA_SRATIONAL
      WhiteBalanceGain_B; //  MF_CAPTURE_METADATA_WHITEBALANCE_GAINS
...
typedef struct _METADATA_FACEDATA {
  ...
  ULONG BlinkScoreLeft;  // [0, 100]. 0 indicates no blink for the left eye. 100
                         // indicates definite blink for the left eye
  ULONG BlinkScoreRight; // [0, 100]. 0 indicates no blink for the right eye.
                         // 100 indicates definite blink for the right eye
  ULONG Reserved;        // Pad for 64-bit alignment.
  BOOL FacialExpression; // Is the face smiling?
  ULONG FacialExpressionScore; // [0, 100]. 0 indicates no such facial
                               // expression as identified. 100 indicates
                               // definite such facial expression as defined
} METADATA_FACEDATA, *PMETADATA_FACEDATA;

typedef struct _CAMERA_METADATA_FACEHEADER {
  KSCAMERA_METADATA_ITEMHEADER Header;
  UINT32 Count;     // Number of detected faces.
  UINT64 Flags;     // If KSCAMERA_EXTENDEDPROP_FACEDETECTION_BLINK or _SMILE
                    // bitflags are set, characterization metadata is valid.
...
} KS_CAMERA_EXTRINSICS_CALIBRATEDTRANSFORM,
    *PKS_CAMERA_EXTRINSICS_CALIBRATEDTRANSFORM;
...
} KS_CAMERAINTRINSIC_PINHOLECAMERAMODEL,
    *PKS_CAMERAINTRINSIC_PINHOLECAMERAMODEL;
...
typedef struct _KS_PINHOLECAMERAINTRINSIC_INTRINSICMODEL {
  ...
  KS_CAMERAINTRINSIC_DISTORTIONMODEL DistortionModel;
    ...
    *PKS_PINHOLECAMERAINTRINSIC_INTRINSICMODEL;
...
DEFINE_GUIDSTRUCT("2A3C5205-F9C8-4F78-B110-43D534C0CA64",
                  KSPROPERTYSETID_CameraAttributes);
#define KSPROPERTYSETID_CameraAttributes                                       \
  DEFINE_GUIDNAMED(KSPROPERTYSETID_CameraAttributes)
...
typedef struct _KSCAMERA_ATTRIBUTES_HEADER {
