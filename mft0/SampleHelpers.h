...
public:
  _Acquires_lock_(m_pLock) explicit CAutoLock(CRITICAL_SECTION *pLock)
      : m_pLock(pLock) {
    EnterCriticalSection(m_pLock);
  }

  _Releases_lock_(m_pLock) ~CAutoLock() { LeaveCriticalSection(m_pLock); }
};

class MediaBufferLock {
public:
  MediaBufferLock(_In_ IMFMediaBuffer *pBuffer) : m_bLocked(false) {
    m_spBuffer = pBuffer;
  }

  HRESULT
  LockBuffer(_Outptr_result_bytebuffer_to_(*pcbMaxLength, *pcbCurrentLength)
                 BYTE **ppbBuffer,
             _Out_opt_ DWORD *pcbMaxLength, _Out_opt_ DWORD *pcbCurrentLength) {
    if (!m_spBuffer) {
      return E_INVALIDARG;
    }
