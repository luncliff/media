...
HRESULT make_test_graphics_device(com_ptr<ID3D11Device>& device, com_ptr<ID3D11DeviceContext>& context,
                                  D3D_FEATURE_LEVEL* levels, uint32_t num_levels) {
    D3D_FEATURE_LEVEL level{};
    return D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, NULL,
                             D3D11_CREATE_DEVICE_DEBUG | D3D11_CREATE_DEVICE_VIDEO_SUPPORT, levels, num_levels,
...
TEST_CASE("ID3D11Device", "[!mayfail]") {
    ...
    SECTION("11 without level") {
        REQUIRE(make_test_graphics_device(device, context, NULL, 0) == S_OK);
    ...
    SECTION("11.1") {
        ...
        REQUIRE(make_test_graphics_device(device, context, levels, 1) == S_OK);
    ...
    SECTION("11.2") {
        ...
        REQUIRE(make_test_graphics_device(device, context, levels, 2) == S_OK);
        REQUIRE(device->GetFeatureLevel() >= D3D_FEATURE_LEVEL_11_1);
...
SCENARIO("MFTransform with ID3D11Device", "[dxva][!mayfail]") {
    ...
    com_ptr<ID3D11Device> device{};
    com_ptr<ID3D11DeviceContext> context{};
    REQUIRE(make_test_graphics_device(device, context, nullptr, 0) == S_OK);

    UINT device_manager_token{};
    com_ptr<IMFDXGIDeviceManager> device_manager{};
    REQUIRE(MFCreateDXGIDeviceManager(&device_manager_token, device_manager.put()) == S_OK);
    REQUIRE(device_manager->ResetDevice(device.get(), device_manager_token) == S_OK);
    ...
    GIVEN("CLSID_CMSH264DecoderMFT") {
        ...
        const auto hr = configure_D3D11_DXGI(transform.get(), device_manager.get());
        REQUIRE(hr == S_OK);
    }
    GIVEN("CLSID_MSMPEGDecoderMFT") {
        REQUIRE(make_transform_video(transform.put(), CLSID_MSMPEGDecoderMFT) == S_OK);
        const auto hr = configure_D3D11_DXGI(transform.get(), device_manager.get());
        REQUIRE(hr == E_FAIL);
    }
    // DSP
    ...
    GIVEN("CLSID_CColorControlDmo") {
        REQUIRE(make_transform_video(transform.put(), CLSID_CColorControlDmo) == S_OK);
        const auto hr = configure_D3D11_DXGI(transform.get(), device_manager.get());
        REQUIRE(hr == E_NOTIMPL);
    }
}
...
TEST_CASE("ID3D11VideoDevice", "[dxva][!mayfail]") {
    ...
    REQUIRE(make_test_graphics_device(device, context, levels, 3) == S_OK);
    configure_multithread_protection(device.get());
    ...
    REQUIRE(device->QueryInterface(video_device.put()) == S_OK);
    com_ptr<ID3D11VideoContext> video_context{};
    REQUIRE(context->QueryInterface(video_context.put()) == S_OK);
    ...
    SECTION("Video Decoder") {
        ...
        for (auto i = 0u; i < num_profile; ++i) {
            ...
            if (auto hr = video_device->GetVideoDecoderProfile(i, &desc.Guid); FAILED(hr))
                FAIL(to_readable(hr));

            desc.SampleWidth = desc.SampleHeight = 600;
            desc.OutputFormat = DXGI_FORMAT_NV12;
            BOOL supported = FALSE;
            if (auto hr = video_device->CheckVideoDecoderFormat(&desc.Guid, desc.OutputFormat, &supported); FAILED(hr))
                FAIL(to_readable(hr));
            if (supported == false) {
            ...
            UINT num_config = 0;
            if (auto hr = video_device->GetVideoDecoderConfigCount(&desc, &num_config); FAILED(hr))
                FAIL(to_readable(hr));

            while (num_config--) {
                D3D11_VIDEO_DECODER_CONFIG config{};
                if (auto hr = video_device->GetVideoDecoderConfig(&desc, num_config, &config); FAILED(hr))
                    FAIL(to_readable(hr));
                com_ptr<ID3D11VideoDecoder> video_decoder{};
                if (auto hr = video_device->CreateVideoDecoder(&desc, &config, video_decoder.put()); FAILED(hr)) {
                    print_error(hr, "ID3D11VideoDecoder::CreateVideoDecoder");
                    spdlog::debug(" - D3D11_VIDEO_DECODER_DESC");
                    spdlog::debug("   guid: {}", to_readable(desc.Guid));
...
SCENARIO("CLSID_VideoProcessorMFT with ID3D11Device", "[dxva][!mayfail]") {
    auto on_return = media_startup();
    ...
    // 1920 * 1080 is the maximum resolution for DXVA
    com_ptr<IMFMediaType> input_type{};
    REQUIRE(make_video_RGB32(input_type.put()) == S_OK);
    REQUIRE(MFSetAttributeSize(input_type.get(), MF_MT_FRAME_SIZE, 1920, 1080) == S_OK);
    ...
    for (auto candidate : try_output_available_types(transform, ostream, type_index)) {
        if (auto hr = MFSetAttributeSize(candidate.get(), MF_MT_FRAME_SIZE, 1920, 1080); FAILED(hr))
            FAIL(to_readable(hr));
